...
const OWNER_ID = 6245574035 & 8195158525;
        ...
        if (data === 'menu_credits') {
            ...
            const creditsKeyboard = {
                inline_keyboard: [
                    ...
                    [{ text: 'UPLOADERS', callback_data: 'credits_uploaders' }, { text: 'BACK', callback_data: 'menu_main' }]
                ]
            };
...
bot.onText(/\/marry/, async (msg) => {
    ...
    const waifu = await getRandomWaifu([1, 15], [9, 11]);
    if (!waifu) {
...
bot.onText(/\/dart\s+(\d+)/, async (msg, match) => {
    ...
    if (user.berries < amount) {
        return sendReply(msg.chat.id, msg.message_id, `❌ Insufficient cash! You have ${user.berries} 💸 ᴄᴀꜱʜ`);
    }
    ...
    const win = Math.random() < 0.30; // 30% win chance

    if (win) {
        const winAmount = amount * 3; // Triple on win
        await pool.query('UPDATE users SET berries = berries + $1 WHERE user_id = $2', [amount * 2, userId]); // Net gain is 2x
        await saveUserDataToFile(userId);
        sendReply(msg.chat.id, msg.message_id, `🎯 <b>Bullseye!</b> You won <b>${winAmount} 💸 ᴄᴀꜱʜ</b>! (Net: +${amount * 2} 💸)`);
    } else {
        await pool.query('UPDATE users SET berries = berries - $1 WHERE user_id = $2', [amount, userId]);
        await saveUserDataToFile(userId);
        sendReply(msg.chat.id, msg.message_id, `💔 Missed! You lost <b>${amount} 💸 ᴄᴀꜱʜ</b>.`);
    }
});
...
bot.onText(/\/dinfo/, async (msg) => {
    sendReply(msg.chat.id, msg.message_id, '🜲ᴀ×ᴡ 𓆩🖤𓆪');
});
...
bot.onText(/\/upload/, async (msg) => {
    ...
    if (!msg.reply_to_message || !msg.reply_to_message.photo || !msg.reply_to_message.caption) {
...
bot.onText(/\/goal(?:\s+(.+))?/, async (msg, match) => {
    ...
    if (!match || !match[1]) {
        const goalData = await pool.query('SELECT * FROM user_goals WHERE user_id = $1', [userId]);
        
        if (goalData.rows.length === 0) {
            return sendReply(msg.chat.id, msg.message_id, '🎯 No goal set! Use /goal <description> to set one.');
        }

        const goal = goalData.rows[0];
        return sendReply(msg.chat.id, msg.message_id, `🎯 Your Goal: ${goal.description}\n\nSet on: ${new Date(goal.created_at).toLocaleDateString()}`);
    }

    const description = match[1].trim();
    await pool.query(
        'INSERT INTO user_goals (user_id, description) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET description = $2, created_at = NOW()',
        [userId, description]
    );

    sendReply(msg.chat.id, msg.message_id, `✅ Goal set: ${description}`);
});
...
bot.onText(/\/gunban\s+(.+)/, async (msg, match) => {
    ...
    sendReply(msg.chat.id, msg.message_id, `✅ ${target.targetName} has been unbanned!`);
        ...
        return sendReply(msg.chat.id, msg.message_id, '❌ Reply to a photo with the correct caption format!\n\n📤 <b>Upload Format Guide</b>\n\nUser sends photo with caption in this exact format (each on a new line):\n\n<code>Name - \nAnime - \nRarity - \nGender - </code>\n\n<b>Example:</b>\n<code>Name - Saitama\nAnime - One Punch Man\nRarity - 1\nGender - Male</code>\n\nThen reply to that message with /upload');
    }
    ...
    if (isNaN(rarity) || rarity < 1 || rarity > 15) {
        return sendReply(msg.chat.id, msg.message_id, '❌ Rarity must be a number between 1 and 15!');
    }
...
bot.on('callback_query', async (query) => {
    if (query.data.startsWith('bazaar_buy_')) {
        ...
        bot.answerCallbackQuery(query.id, { text: 'THANK YOU FOR BUYING ❤️🔥', show_alert: true });
